...
- 简单 / 进阶 / 探索分别为 14 / 195 / 338 个拆装步骤；模型仍保留 595 个独立实体。
- 所有步骤都绑定真实模型对象，并提供名称、作用、原理和进阶说明。
- 选择工具后可自由选择零件，拖入对应分类托盘；组装时也不限制先后顺序。
- 本地保存拆解等级、工具、准确的已拆零件集合、组装模式和讲解开关。
...
| 拆解等级 | 拆装粒度 | 步骤数 |
| --- | --- | ---: |
| 简单 | 车轮、前叉、制动、传动等主要总成 | 14 |
| 进阶 | 同类零件成组，例如 32 根辐条作为一组 | 195 |
| 探索 | 普通零件逐件操作；辐条、链节等高重复零件按整体操作 | 338 |

探索等级只增加拆分粒度。链条显示全部链节，但高重复链节作为一组通过快拆扣拆卸；辐条、辐条帽等高重复零件同样整体操作；密封轴承作为维修单元拆卸并可用剖视讲解内部结构；焊接、粘接、硫化和铆死结构不作为常规拆装操作。刹车油流程计划在后续覆盖所有等级。
...
当前验证基线：595 个零件对象、585 个网格对象、121,498 顶点、117,262 面；LOD1 为 14 个对象 / 76,251 面，LOD2 为 1 个对象 / 26,191 面。刹车油管和变速线管沿车架外走线，水壶架螺栓固定于下管安装座，坐垫采用带减压凹槽的曲面模型。
...
6. 点击顶部“拆装零件”后，选好工具再拖动零件；空白处仍可旋转，桌面端右键也可旋转。“整车归位”恢复完整取景，不修改拆装进度。
   右下角“画面平移”四向按钮按屏幕方向移动画面，可配合旋转和缩放；“中”只取消平移，保留旋转和缩放。“整车归位”同时重置三者。
7. 左侧“工具与拆解等级”和右侧“零件百科”可收起，底部分类区压缩为两行。相机根据中间可用区域自动构图；“查看收纳”将已拆零件和 3D 展示托盘纳入取景，进入组装时自动切换到此视图。
8. Windows Editor 内置本地中文讲解：点击零件、成功拆装或点击“重听当前零件”即可播报；关闭讲解会停止当前声音。
