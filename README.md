...
10. 顶部“运转演示”可在整车组装完整时启动固定 36T / 24T 的原地踩踏；默认前后轮按相同目标转速转动，不额外生成脚撑或支架。按住左刹把仅使后轮及联动的踩踏传动逐渐减速，按住右刹把仅使前轮逐渐减速；松开任一刹把后，其对应车轮逐渐加速至设定速度。顶部可暂停、继续和结束，左侧可在 30–90 转/分间调速。进入拆装、爆炸或收纳视图会结束演示并恢复静态姿态。演示链条单独显示为绕过两只后拨导轮的闭合链路，原有 110 个可拆链节及 595 个实体 ID 保持不变。
