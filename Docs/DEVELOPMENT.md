...
- Source 117,262 面、LOD1 76,251 面、LOD2 26,191 面。
- Simple / Standard / Advanced 为 14 / 195 / 338 步；探索等级仍覆盖 595 个模型对象。
...
- “拆装零件”模式：选工具后拖零件到对应底部分类区，变绿时松开；拖空白处旋转。“查看收纳”将已拆零件和 3D 托盘完整纳入中央区域，组装仍从这些 3D 展示位置拖回工作区，进入组装会自动调整取景。
...
- 左侧状态应显示“系统中文语音 · 就绪”。点击零件或“重听当前零件”后，日志依次出现 `MECH_MASTER_NARRATION_SPEAKING` 和 `MECH_MASTER_NARRATION_FINISHED`；若系统静音或输出设备不正确，事件成功也不能证明扬声器可听见，需人工试听。
...
- 三档计数为 14 / 195 / 338。
- 338 个探索步骤覆盖全部 595 个模型对象，高重复零件按集合绑定。
- 文档统计与运行清单一致。
- 新资料与第三方资产记录来源和许可。
